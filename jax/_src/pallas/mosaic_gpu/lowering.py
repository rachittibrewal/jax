...
def _handle_indexing(
  ...
  indexer_offset = 0
  for indexer_idx in indexer_idxs:
    ...
    new_transforms_rev = []
    for t in reversed(new_transforms[:indexer_idx - indexer_offset]):
      indices, new_t = t.untransform_index(indices)
      new_transforms_rev.append(new_t)
    sliced_ref = mgpu.memref_slice(sliced_ref, indices)
    new_transforms = [*reversed(new_transforms_rev),
                      *transforms[indexer_idx + 1:]]
    indexer_offset += 1
